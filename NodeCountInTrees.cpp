...
class BinaryTree     //  Class created for Binary Tree Implementation ( Non-Linear Data Structure )
{
    ...
    {   // Recursive Call for both left and right subtree and adding 1 for the current node
        if(root == NULL)
            return 0;
        return CountNodesInTree(root -> left) + CountNodesInTree(root -> right) + 1;
    }
    int SumOfNodesInTree(Node* root)
    {  // Recursive Call for both left and right subtree and adding node value
        if(root == NULL)
            return 0;
...
int main()
{
    ...
    x = binarytree.CountNodesInTree(root);
    cout << x << endl;
    x = binarytree.SumOfNodesInTree(root);
    cout << "The Sum of Nodes in the Tree : ";
    cout << x << endl;
